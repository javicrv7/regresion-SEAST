...
x <- 1:nrow(GradosTeleco)
training.posiciones <- sample(x, replace = FALSE, 68)
...
resumenerrores <- errores %>% group_by(caracteristicas) %>%
  summarise(error_cuadratico = mean(errores^2, na.rm=TRUE),
            error_menor_cuarto = (sum(abs(errores) < 0.25, na.rm=TRUE) / n())*100,
            error_menor_decima= (sum(abs(errores) < 0.1, na.rm=TRUE) / n())*100)
...
training.lm.completo <- lm(data=GradosTeleco.training, notamedia ~ .)
```
### Predicción de la nota media para el conjunto test
...
> No, ya que con las asignaturas de la PAU se consigue ya un error pequeño, y no es necesario incluir las asignaturas de la UPCT.
...
df <- data.frame(GradosTeleco.test$PAU_CALIFICACION_NOTA_MATERIA, GradosTeleco.test$notamedia, test.predict.completo)

ggplot(data = df, aes(x = GradosTeleco.test.PAU_CALIFICACION_NOTA_MATERIA, y = GradosTeleco.test.notamedia)) + geom_point() + geom_point(aes(y = test.predict.completo), colour = "red")
